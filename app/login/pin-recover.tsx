    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        <TouchableOpacity style={styles.back} onPress={() => email ? router.replace({ pathname: '/login/pin-login', params: { email } }) : router.replace('/login/pin-login')}>
          <Text style={[styles.backText, { color: colors.primary }]}>Back to PIN</Text>
        </TouchableOpacity>
        <Text style={[styles.title, { color: colors.text }]}>Recover PIN</Text>
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>Enter your email to receive a 6-digit code</Text>
        ...
        {sentOnce && (
          <>
            <Text style={[styles.subtitle, { color: colors.textSecondary, marginTop: 24 }]}>Enter the 6-digit code</Text>
